...
struct MorningWidgetEntryView: View {
    ...
    var body: some View {
        ZStack {
            Image(ImageService.dailyBackgroundImage(for: entry.date))
                .resizable()
                .scaledToFill()
...
class ImageService {
    static func dailyBackgroundImage(for date: Date) -> String {
        let imageNames = ["photo1", "photo2", "photo3", "photo4", "photo5", "photo6", "photo7", "photo8", "photo9", "photo10", "photo11"]
